...
> This file is auto-generated by `skills/scripts/extract-summaries.ps1` - **do not hand-edit**.
> To change summaries, edit the frontmatter `description` of the corresponding module's `SKILL.md`, then re-run the script.
...
| [apk-reverse](apk-reverse/SKILL.md) | Use when reversing Android APKs in a CLI environment. Covers APK unpacking, Java decompilation, smali modification, repackaging, Frida dynamic hooking, and on-demand switching to so/native analysis. Prefer locally installed jadx, apktool, frida, adb, ida-reverse, radare2. |
...
| [binary-diff](binary-diff/SKILL.md) | Cross-version symbol migration and binary diffing. Use when you have old-version symbols/reversing results and need to migrate quickly to a new version. |
| [browser-automation](browser-automation/SKILL.md) | Unified automation entry point. Covers browser automation (Playwright) and Windows desktop-app automation (OpenReverse). |
...
| [dotnet-reverse](dotnet-reverse/SKILL.md) | .NET / C# binary reversing. When the target is a .NET assembly (PE header with CLR, managed .exe/.dll programs), C# build output (incl. NativeAOT), red-team Sharp* tools (Rubeus / SharpHound / SharpHound, etc.), .NET obfuscated programs (ConfuserEx / Sma... |
| [edr-bypass-re](edr-bypass-re/SKILL.md) | Reverse defensive implementations -> targeted red-team bypass. First reverse the hook tables, ETW providers, and AMSI implementations of EDR / Defender / AV, |
...
| [firmware-pentest](firmware-pentest/SKILL.md) | Firmware / IoT pentest chain. From receiving a blob of .bin / .img, run the full loop: reverse -> extract -> emulate -> exploit. |
...
| [ida-reverse](ida-reverse/SKILL.md) | IDA Pro reversing analysis companion skill. Use it whenever the user mentions reversing, decompiling, analyzing binaries/PE/ELF/APK/DLL/SO, cracking, password hunting, vulnerability analysis, malware analysis, firmware analysis, or needs to analyze exe/dll/so/elf/macho/sys files. |
...
| [js-reverse](js-reverse/SKILL.md) | Use when reversing frontend JavaScript with js-reverse-mcp; suits signature-chain location, page-observation forensics, runtime sampling, local environment reproduction with evidence-based output. Prefer adapting the js-reverse_* tools in the current environment; link jshookmcp when a stronger browser/CDP/Hook surface is needed. |
...
| [patch-diff-exploit](patch-diff-exploit/SKILL.md) | N-day patch diffing to exploitation. Infer the vulnerable point from vendor-published patches, write a PoC, and turn it into a usable attack module. |
| [pentest-tools](pentest-tools/SKILL.md) | Active pentest toolchain. Covers information gathering, port scanning, vulnerability scanning, web pentest, SQL injection, directory brute force, password cracking, and more. |
| [src-hunter](pentest-tools/src-hunter/SKILL.md) | Hands-on SRC / crowdtesting / Bug bounty vulnerability-hunting workflow skill. Includes: 5-stage methodology (intake -> recon -> enum -> hunt -> report), 19 attack-class playbooks (SQLi/XSS/RCE/SSRF/IDOR/CSRF/Path Traversal/File Upl... |
...
| [pwn-chain](pwn-chain/SKILL.md) | End-to-end engineering method from reversing to a working exploit. |
...
For ambiguous cases read the full matrix in `skills/routing.md`; multi-type CTF tasks go through `CTF-Sandbox-Orchestrator/`.
