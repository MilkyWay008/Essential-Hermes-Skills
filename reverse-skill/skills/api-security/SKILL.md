...
```text
Active discovery:
□ Vespasian: headless browser crawling → auto-generates OpenAPI 3.0 / GraphQL SDL specs
...
GraphQL introspection (three-level attempts):
  ...
  2. Minimal query (bypass WAF full-block rules)
...
□ mutation via GET request to bypass auth (CSRF)
...
□ Normal user executes admin APIs
...
Batched queries → DoS with 10+ simultaneous queries
...
GraphiQL/Playground exposure → publicly accessible IDE risk
...
□ GraphQL batched deep-nested query DoS
...
□ GraphQL nested traversal to access unauthorized data
