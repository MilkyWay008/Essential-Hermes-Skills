...
2. Continue checking Evidence records for reproducible commands and sources
3. Export the current review result and attach it to the milestone report
4. Switch to JSON output for CI or other review tools
...
1. Fill in missing Evidence and keep original commands
2. Re-run the review after binding candidate Findings to Evidence
3. Add P-ids and Path steps for call chains or attack chains
4. Generate a Markdown handoff summary
5. Switch back to the PRIMARY skill and continue analysis
...
1. Fix the hash mismatch or replace the tampered working copy
2. Add SHA-256 and artifact_path for unfixed original files
3. Proceed to the report generation phase
...
1. Hand the passed review result to `docs-generator/` to generate the formal report
...
- [ ] Do all Paths include a valid path_type and Evidence references?
- [ ] Was hash verification performed, or was the reason for skipping it recorded?
- [ ] Was the review re-run in strict mode and the result saved?
