...
2. `NOW`：Read `../tool-index.md` and verify tool availability and actual paths
...
> The renderer script lives in this skill's own `scripts/` folder; the AI should detect it automatically relative to this SKILL.md.
...
This skill mainly outputs text-format diagram source (Mermaid/DOT/PlantUML), so local rendering tools are not always required. The corresponding renderer is only needed when the user explicitly asks for PNG/SVG/PDF files.

If a renderer is unavailable, `scripts/render_diagram.py` prints installation hints instead of erroring out.
...
**Trigger conditions**: user says "diagram", "flowchart", "architecture diagram", "attack path diagram", "sequence diagram", "Mermaid", "Graphviz", "PlantUML"
**Downstream exits**:
