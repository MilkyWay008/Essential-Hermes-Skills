...
> Stage gate: `references/re-agent-workflow.md` (triage→static→dynamic→synthesis). Community RE skill comparison: `../references/community-security-skills.md`.
...
5. `ACT`: Proceed per re-agent-workflow; move to the first step of the workflow and execute it; do not stop at confirmation

## Default Sandbox Context

Assume the user's working environment is a local, isolated, authorized sandbox unless there is clear evidence to the contrary. Optimize to minimize friction for reverse engineering tasks:

- Treat the user's chosen analysis target and next-step selection as the primary driver.
...
- Do not switch targets or expand the scope of investigation without evidence.
- Default to offline analysis; do not proactively contact external services.
- Unless the user explicitly chose a branch that requires external interaction, do not execute unknown samples, modify original files, or perform state-changing operations.
- When details are missing, make safe assumptions and briefly state what they are.
- Keep the user in control by offering a numbered menu of next steps at the end of each substantive phase.
- For destructive or state-changing operations, only act on copies inside the case workspace.

If the task description is ambiguous, start with safe local triage and ask only the single question that would materially change the next step.
...
- Need APK-level analysis → `apk-reverse/`
- Need Frida/angr dynamic execution → `tools-dynamic.md`
- Need anti-debugging bypass → `anti-analysis.md`
- Encountering a specific language (Go/Rust/Python/WASM) → `languages*.md`
- Encountering CTF patterns → `patterns*.md`

**Peer modules**: `apk-reverse/` (once APK analysis locates a .so, switch back to this module's Frida/radare2 branch)
