...
> The complete OLLVM deobfuscation workflow, variant ecosystem (Hikari/Polaris/O-MVLL/Tigress/Hodur etc.), and community tool survey are in [ollvm-deobfuscation.md](references/ollvm-deobfuscation.md).
...
**Modern variants (community-active 2026):** Hikari (Anti Class Dump/String Encryption/Indirect Branch), Polaris (formerly Pluto, includes a Trap Angr pass that breaks angr), O-MVLL (Python-driven, common for Android hardening), Arkari (goron-based; indirect jumps can be countered with read-only data segments), amice (Rust; includes VM Flatten that needs VM reversing rather than deflat). See ollvm-deobfuscation.md §1 for variant identification.

**Deobfuscation tools (by community activity):**
- **obpo-plugin** (629⭐, IDA microcode+concolic cloud plugin, most effective): https://github.com/obpo-project/obpo-plugin
- **ollvm-breaker** (441⭐, Binary Ninja, Android .so field-proven): https://github.com/amimo/ollvm-breaker
...
- **DeObfBR** (96⭐, BR indirect-branch obfuscation specialist): https://github.com/Mrack/DeObfBR
- **D-810** (original, less maintained — prefer d810-ng): pattern-based deobfuscation, MBA simplification
...
# obpo: right-click the dispatcher → OBPO → Mark and process function (requires network)
# ⚠️ Pluto/Polaris's Trap Angr pass breaks angr → use d810-ng/Unicorn instead
```
