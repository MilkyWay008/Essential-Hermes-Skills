...
**Full analysis**: [phishing-case-study.md](phishing-case-study.md) — *created alongside this note when the case study is written (template slot; not present until filled)*
...
## Pre-Analysis Triage: File Masquerading and Name Deception
...
| Masquerading Extension | Actual Type | Purpose |
|---------|---------|------|
| `.sh` | ELF binary | Looks like a script, lowers suspicion |
| `.txt` | PE/ELF | Bypasses simple file-type filters |
| `.jpg`/`.png` | Executable or archive | Hidden inside an image |
| `.dll` | Actually a .NET assembly | Obscures the analysis direction |
| `.so` | Actually an encrypted payload | Needs decryption first |
...
**"DriverLoader" doesn't necessarily load drivers; "Updater" doesn't necessarily update.**

Common name deception:

| Name Implies | Actual Behavior |
|-----------|---------|
...
- Watch syscalls like `mmap`, `ptrace`, `/proc/self/mem`
- If you see "load driver" without `insmod`/`init_module` calls, the name is misleading

### Dynamic Supplement When Static Analysis Isn't Enough

Pure static analysis only reveals the code skeleton. These scenarios require dynamic analysis:

| Scenario | Recommended Dynamic Method |
|------|-------------|
| Code has decryption/decompression logic | Breakpoint after decryption, dump the plaintext |
| Heavy indirect calls (function-pointer tables) | Track actual calls with strace/ltrace |
| Suspected anti-debugging | strace first to see ptrace calls |
| Embedded shellcode/payload | Emulate execution in QEMU user mode |
| Unknown network protocol | Capture traffic with tcpdump/Wireshark |

```bash
# strace to trace syscalls (key focus)
...
# Check anti-debugging: see if it ptrace self-traces
strace ./binary 2>&1 | grep ptrace
# If you see ptrace(PTRACE_TRACEME, ...) it has anti-debugging
```

### Common Patterns in Process-Injection / Packed Samples

Samples like `LinYuDriverLoader` typically:

1. **Don't actually load a kernel driver** (needs root, which most scenarios lack)
...
4. **Anti-debugging protection**:
   ...
   - Time checks (comparing `clock_gettime` before/after)
...
5. If decryption logic exists -> run dynamically to just after decryption and dump
6. If anti-debugging exists -> patch it out or bypass with LD_PRELOAD first
```
