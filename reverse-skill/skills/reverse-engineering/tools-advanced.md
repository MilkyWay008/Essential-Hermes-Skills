...
The modern maintained version of D-810 (Next Generation), integrating a **Z3 SMT** solver; covers OLLVM / Tigress / Hodur(PlugX) / Approov and other variants.
...
- Hacker's Delight bit-operation equivalences
...
Built on Hex-Rays **microcode** + data-flow tracking + program slicing + concolic execution. Widely regarded as one of the strongest in the community.

```text
- Optimizes decompiled output at the microcode layer (not by patching ASM)
...
No IDA dependency; based on Miasm symbolic execution with BFS multi-layer processing; supports x86/x64 on Win/Linux.
...
Binary Ninja de-flattening; the repo ships an Android-packed sample `libvdog.so` for practice. Source: amimo/ollvm-breaker

### DeObfBR - BR obfuscation specialist

Purpose-built to remove Goron/Arkari-style BR (indirect-branch) obfuscation. Simple trick: making the data segment read-only partially counters it. Source: Mrack/DeObfBR

### deollvm — ARM64 Unicorn

Alternative for ARM64 .so when IDA is unavailable, based on Unicorn. Source: GeT1t/deollvm
